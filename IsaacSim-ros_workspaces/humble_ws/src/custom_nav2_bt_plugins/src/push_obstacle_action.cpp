...
#include "custom_nav2_bt_plugins/push_obstacle_action.hpp"
...
PushObstacleAction::PushObstacleAction(
...
void PushObstacleAction::buildSequence()
{
  // Same approach/reach joint poses as FixedPickObjectAction, reused here to
  // put the arm where an object would be picked - but the gripper stays OPEN
  // and the arm just HOLDS there (no grasp, no retreat). Whatever pushing
  // happens is done by the robot base driving into it while the arm holds
  // this pose (see DriveOnHeading in the XML, right after this node).
  const std::array<double, 4> P_HOME  {{-0.0015339807878856412, -1.0461748973380072,
      1.0753205323078345, 0.009203884727313847}};
  // No separate pre_push/push stages anymore - go straight from home to one
  // target pose and hold it there. Was originally the SAME reach as
  // FixedPickObject's P_PRE_PICK (approached, then reached deeper to
  // P_PICK depth for "push") - live-tested via direct FollowJointTrajectory
  // goals (bypassing the full scenario to iterate fast): every depth from
  // that original P_PRE_PUSH through P_PICK depth lifted the rear wheels
  // enough to tilt the chassis-mounted lidar into the floor (seen as a
  // phantom obstacle -> immediate stop). This is 50% of the way from
  // P_HOME to the old P_PRE_PUSH - confirmed live to be the deepest reach
  // that stays stable, and reaching it directly (no intermediate pose) is
  // enough to push - no need for a separate deeper "push" step.
  const std::array<double, 4> P_PUSH  {{-0.0107379, -0.1043107, 0.4747671, -0.4172428}};
  ...
  steps_.push_back({Kind::ARM,     P_PUSH, 0.0,            push_hold_time_, "push"});
}

BT::NodeStatus PushObstacleAction::onStart()
{
  ...
  getInput("push_hold_time", push_hold_time_);
  getInput("server_timeout", server_timeout_);
  getInput("step_timeout", step_timeout_);

  if (arm_joint_names_.empty()) {
    RCLCPP_ERROR(node_->get_logger(), "PushObstacle: arm_joint_names is empty");
  ...
  RCLCPP_INFO(
    node_->get_logger(),
    "PushObstacle: starting push sequence (%zu steps)", steps_.size());
  return BT::NodeStatus::RUNNING;
}

BT::NodeStatus PushObstacleAction::onRunning()
{
  ...
  switch (phase_) {
    case Phase::SEND: {
      ...
      RCLCPP_INFO(
        node_->get_logger(), "PushObstacle: step %zu/%zu '%s' sent",
    ...
    case Phase::WAIT_RESULT: {
      ...
      if (done) {
        ...
        if (!ok) {
          RCLCPP_ERROR(
            node_->get_logger(), "PushObstacle: step '%s' did not succeed", step.label);
          return BT::NodeStatus::FAILURE;
        }
    ...
    case Phase::POST_DELAY: {
      if ((node_->now() - delay_start_time_).seconds() >= step.post_delay) {
        RCLCPP_INFO(
          node_->get_logger(), "PushObstacle: step %zu/%zu '%s' done",
          step_idx_ + 1, steps_.size(), step.label);
        step_idx_++;
        phase_ = Phase::SEND;
        step_start_time_ = node_->now();
        if (step_idx_ >= steps_.size()) {
          RCLCPP_INFO(node_->get_logger(), "PushObstacle: push sequence complete");
          return BT::NodeStatus::SUCCESS;
        }
      }
      return BT::NodeStatus::RUNNING;
...
void PushObstacleAction::onHalted()
{
  RCLCPP_INFO(node_->get_logger(), "PushObstacle: halted, cancelling any active goal");
  cancelActive();
}

BT::NodeStatus PushObstacleAction::fail(const char * why)
{
  RCLCPP_ERROR(node_->get_logger(), "PushObstacle: %s", why);
  cancelActive();
  return BT::NodeStatus::FAILURE;
}

void PushObstacleAction::cancelActive()
{
...
{
  factory.registerNodeType<custom_nav2_bt_plugins::PushObstacleAction>("PushObstacle");
}
