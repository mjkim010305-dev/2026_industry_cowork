...
#include "custom_nav2_bt_plugins/fixed_pick_object_action.hpp"
...
FixedPickObjectAction::FixedPickObjectAction(
...
void FixedPickObjectAction::buildSequence()
{
  ...
  // const std::array<double, 4> P_PRE_PICK {{-0.019941750242513337, 0.8375535101855601,
  //     -0.12578642460662257, -0.8436894333371027}};
  // // const std::array<double, 4> P_PICK     {{-0.02454369260617026, 1.2885438618239387,
  // //     -0.7731263170943632, -0.4816699673960913}};
  // const std::array<double, 4> P_PICK     {{-0.02454369260617026, 1.2885438618239387,
  //     -0.7731263170943632, -0.7816699673960913}};
  // const std::array<double, 4> P_LIFT     {{-0.02454369260617026, 0.518485063053467,
  //     0.04908738521234052, -0.6181942575179133}};
  // const std::array<double, 4> P_CARRY    {{-0.039883500485026674, -0.9188544919434991,
  //     0.48013598600820567, 0.4724660826687775}};
...
// ]
  ...
  const std::array<double, 4> P_HOME     {{-0.0015339807878856412 - 1.5707963268, -1.0461748973380072,
      1.0753205323078345, 0.009203884727313847}};
  const std::array<double, 4> P_PRE_PICK {{-0.019941750242513337 - 1.5707963268, 0.8375535101855601,
      -0.12578642460662257, -0.8436894333371027}};
  const std::array<double, 4> P_PICK     {{-0.02454369260617026 - 1.5707963268, 1.2885438618239387,
      -0.7731263170943632, -0.4816699673960913}};
  const std::array<double, 4> P_LIFT     {{-0.02454369260617026 - 1.5707963268, 0.0,
      0.0, 0.0}};
  const std::array<double, 4> P_CARRY    {{-0.039883500485026674 - 1.5707963268, 0.0,
      0.0, 0.0}};
  ...
  steps_.push_back({Kind::GRIPPER, none,      grasp_position_, 1.0, "gripper_grasp"});
  steps_.push_back({Kind::ARM,     P_LIFT,    0.0,             3.0, "lift"});
  steps_.push_back({Kind::ARM,     P_CARRY,   0.0,             0.0, "carry"});
}

BT::NodeStatus FixedPickObjectAction::onStart()
{
  ...
  if (arm_joint_names_.empty()) {
    RCLCPP_ERROR(node_->get_logger(), "FixedPickObject: arm_joint_names is empty");
  ...
  RCLCPP_INFO(
    node_->get_logger(),
    "FixedPickObject: starting fixed pick sequence (%zu steps)", steps_.size());
  return BT::NodeStatus::RUNNING;
}

BT::NodeStatus FixedPickObjectAction::onRunning()
{
  ...
  switch (phase_) {
    case Phase::SEND: {
      ...
      RCLCPP_INFO(
        node_->get_logger(), "FixedPickObject: step %zu/%zu '%s' sent",
    ...
    case Phase::WAIT_RESULT: {
      ...
      if (done) {
        ...
        if (!ok) {
          RCLCPP_ERROR(
            node_->get_logger(), "FixedPickObject: step '%s' did not succeed", step.label);
          return BT::NodeStatus::FAILURE;
        }
    ...
    case Phase::POST_DELAY: {
      if ((node_->now() - delay_start_time_).seconds() >= step.post_delay) {
        RCLCPP_INFO(
          node_->get_logger(), "FixedPickObject: step %zu/%zu '%s' done",
          step_idx_ + 1, steps_.size(), step.label);
        step_idx_++;
        phase_ = Phase::SEND;
        step_start_time_ = node_->now();
        if (step_idx_ >= steps_.size()) {
          RCLCPP_INFO(node_->get_logger(), "FixedPickObject: pick sequence complete");
          return BT::NodeStatus::SUCCESS;
        }
      }
      return BT::NodeStatus::RUNNING;
...
void FixedPickObjectAction::onHalted()
{
  RCLCPP_INFO(node_->get_logger(), "FixedPickObject: halted, cancelling any active goal");
  cancelActive();
}

BT::NodeStatus FixedPickObjectAction::fail(const char * why)
{
  RCLCPP_ERROR(node_->get_logger(), "FixedPickObject: %s", why);
  cancelActive();
  return BT::NodeStatus::FAILURE;
}

void FixedPickObjectAction::cancelActive()
{
...
{
  factory.registerNodeType<custom_nav2_bt_plugins::FixedPickObjectAction>("FixedPickObject");
}
