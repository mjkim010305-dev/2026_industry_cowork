...
#ifndef CUSTOM_NAV2_BT_PLUGINS__PUSH_OBSTACLE_ACTION_HPP_
#define CUSTOM_NAV2_BT_PLUGINS__PUSH_OBSTACLE_ACTION_HPP_
...
/**
 * @brief Extend the arm to a fixed "push" pose and HOLD it there.
 *
 * This does NOT push anything by itself - it just gets the arm into position
 * (gripper OPEN, arm reaching out to the same joint pose FixedPickObject
 * would use to approach an object) and stays there when it returns SUCCESS.
 * The actual pushing is done by driving the ROBOT BASE forward into the
 * obstacle (e.g. a DriveOnHeading node placed right after this one in the
 * XML) while the arm remains rigid at this pose, using the extended arm/
 * gripper as a plow. Retracting the arm afterward is a separate node
 * (RetractArm) so the drive can happen in between.
 *
 * Steps: gripper open -> home -> pre_push -> push (hold, ends here)
 *
 * Non-blocking: StatefulActionNode with an internal step machine, same
 * pattern as FixedPickObjectAction / PlaceObjectAction.
 */
class PushObstacleAction : public BT::StatefulActionNode
{
public:
  ...
  PushObstacleAction(const std::string & name, const BT::NodeConfiguration & conf);
  PushObstacleAction() = delete;
  ...
  static BT::PortsList providedPorts()
  {
    return {
      ...
      BT::InputPort<double>(
        "open_position", 0.02501155674647538, "Gripper open command [m] (stays open throughout)"),
      BT::InputPort<double>(
        "max_effort", 0.0, "Gripper max effort (0 = controller default)"),
      BT::InputPort<double>(
        "push_hold_time", 1.0, "Seconds to settle at the push pose before returning SUCCESS"),
      BT::InputPort<double>(
        "server_timeout", 5.0, "Seconds to wait for the two action servers"),
      BT::InputPort<double>(
        "step_timeout", 15.0, "Per-step timeout [s] before the push fails"),
    };
  }
...
#endif  // CUSTOM_NAV2_BT_PLUGINS__PUSH_OBSTACLE_ACTION_HPP_
